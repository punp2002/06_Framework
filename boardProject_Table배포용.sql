...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$uecWLMBBvSUAe6OhQvAVbuYj3goT8ZHlGf.GVhFhoh9hQ/5uORTtS'
WHERE MEMBER_NO = 1;
...
VALUES(1, 2000); -- 1번 회원이 1994번 게시글에 좋아요를 누름
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람 오픈맴버일때는 참가자 타겟번호)
		R.OPEN_MEMBER -- NULL 인 경우 (내가 오픈 맴버가 아닐때는 OPEN_MEMBER 타겟번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
