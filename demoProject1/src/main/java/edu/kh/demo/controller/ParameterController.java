...
public class ParameterController {
	 ...
	 * - 요청 클라이언트의 정보, 제출된 파리미터 등을 저장한 객체
	 * - 클라이언트 요청 시 생성 
	 * 
	 ...
	 * 
	 * value : 전달받은 input 태크의 name 속성값(파라미터 key)
	 * required : 입력된 name 속성값 파라미터 필수 여부 지정 (기본값 true)
	 * -> required=true 인 파리미터가 존재하지 않는다면 400(Bad Request) 에러 발생
	 * -> ""(빈문자열)일 때는 에러 발생 x 
	 * (파라미터가 존재하지 않는 것이 아니라 name 속성값=""로 넘어오기때문에)
	 * 
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							// 형변환이 필요한 경우 파라미터 valuee값이 필수 작성되어있어야함!
							@RequestParam(value="publiser", 
							required = false, 
							defaultValue="kh출판사") String publiser) {
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		// {color=Red, fruit=Apple, productName=아름다운조명, expirationDate=2025-04-30}
