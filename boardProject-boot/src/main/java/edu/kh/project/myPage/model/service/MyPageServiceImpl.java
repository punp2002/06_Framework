...
public class MyPageServiceImpl implements MyPageService{
	...
	// bcrypt 암호화 객체 의존성 주입 (시큐리티 콘피그 참조)
	@Autowired
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// DB에 업데이트
		// SQL에 전달 해야하는 데이터 2개 (암호화한 새비번 encPw, 회원번호 memberNo)
		// -> mapper에 전달할 수 있는 전달인자는 단 1개!
		// -> 묶어서 전달 (parmaMap 재활용)
		
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo + ""); // 1+"		
	...
	public int profile(MultipartFile profileImg, Member loginMember) throws Exception {
		...
		// 업로드한 이미지가 있을 경우
		// - 있을 경우 : 경로 조합 (클라이언트 접근 경로 + 리네임팦일명)
		...
		if(result > 0) {
			...
			// 프로필 이미지를 업애는 update 를 한 경우를 제외
			// -> 업로드한 이미지가 있을 경우
			if(!profileImg.isEmpty()) {
				// 파일을 서버에 저장
				profileImg.transferTo(new File(profileFolderPath + rename));
							// C:/uploadFiles/profile/변경된 파일명
			}
