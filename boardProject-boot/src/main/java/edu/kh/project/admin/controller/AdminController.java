...
public class AdminController {
	...
	@GetMapping("/userJoin")
	public ResponseEntity<?> getUserJoin() {
		try {
			List<Member> list = service.getUserJoin();

			return ResponseEntity.status(HttpStatus.OK).body(list);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null); // 500 에러
		}
	...
	public ResponseEntity<Object> getAdminList() {
		// 성공 시 List<Member> 반환 , 실패 시 String 반환 -> Object
		try {
			List<Member> getAdminList = service.getAdminList();
			return ResponseEntity.status(HttpStatus.OK).body(getAdminList);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("관리자 계정 목록 조회 중 문제 발생 : " + e.getMessage());
		}			
	}
