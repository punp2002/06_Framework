...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        /*
        submit 이벤트 헨들러 함수 내에서 fetch를 이용한 비동기 함수를 실행중 일 때
        e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form이 
        submit되어버린 다음일 수 있음
        -> 비동기 작업중에 form 의 기본 제출 동작 일어나는 가능성이 생김!
        */
    });
}
...
if(profileForm != null){
    ...
    const defaultImageUr = `${window.location.origin}/images/user.png`;
    ...
    imageInput.addEventListener("change", () => {
        ...
        if(file) { // 파일이 선택된 경우
            ...
            if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내 인 경우(정상인 경우)
                ...
                previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뀔일 대비)
            ...
            if(previousFile) { // 이전 파일이 존재한다면
                ...
                // --> input 태크의 files 속성은 FileList만 저장 가능한 형태이기 때문에
    ...
    deleteImage.addEventListener("click", () => {
        ...
        if(profileImg.src !== defaultImageUr) {
            imageInput.value = ""; // input 태그 파일 값 초기화
            profileImg.src = defaultImageUr; // 현재 미리보기를 기본이미지로 변경
            statusCheck = 0; // 삭제 상태 기록
            previousImage = defaultImageUr; // 이전 이미지 기록하는 변수에 기본이미지로 변경
