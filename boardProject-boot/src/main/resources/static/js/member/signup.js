...
// execDaumPostcode호출 함수에 소괄호 붙이면 안됨 클릭시 이벤트 발생하지 않음
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  fetch("/email/checkAuthKey", {
    method : "POST",
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(obj) // JS 객체를 JSON 문자열로 변환하여 전송
  })
  .then(resp => resp.text())   // 서버 응답을 문자열로 반환
  .then(result => {
    // 서버에서 1 또는 0 반환
    if(Number(result) === 0) { // 인증 실패 시
...
signUpForm.addEventListener("submit", e => {
  ...
  for(let key in checkObj) { // checkObj 요소의 key 값을 순서대로 꺼내옴
    ...
    if(!checkObj[key]) { // 현재 접근중인 checkObj[key] 의 value 값이 false 인 경우
